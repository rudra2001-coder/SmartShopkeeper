...
import com.rudra.smartshopkeeper.presentation.viewmodels.ProfitPerCategoryViewModel
...
@Composable
fun ProfitPerCategoryScreen(
    viewModel: ProfitPerCategoryViewModel = hiltViewModel(),
    onBack: () -> Unit
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { BengaliText(text = "ক্যাটাগরি অনুযায়ী লাভ", fontSize = 20.sp, fontWeight = FontWeight.Bold) },
                navigationIcon = {
        ...
        Column(
            ...
            DateRangeSelector(
                ...
                onGenerateReport = { viewModel.loadProfitPerCategory() }
            )
            ...
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(state.profitPerCategory.keys.toList()) { category ->
                    Row(modifier = Modifier.fillMaxWidth()) {
                        BengaliText(text = category, modifier = Modifier.weight(1f))
                        BengaliText(text = "৳${state.profitPerCategory[category]}")
                    }
                }
            }
        }
